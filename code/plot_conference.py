...
def tweets(dat):
    ...
    for i, c in enumerate(['TWT', 'F', 'RT']):
        ax[i].plot(dat.Hours, dat[c], marker = 'o', ms = 5, color = colors['red'], mfc = 'none')

        ax[i].set_xlim(0, dat.Hours.max() + 1)
        ax[i].set_ylabel(c)
    ...
    ax[0].set_title('#' + cf['CONF']['hashtag'] + ': Tweets per Hour',
                    fontdict = {'fontsize': 10, 'fontweight': 100, 'verticalalignment': 'bottom'},
                    color = colors['black'])
    ax[1].set_xlabel('Hours', color = colors['black'])

    fig.patch.set_alpha(0.25)
    plt.savefig('images/Conference_Activity.png', dpi = 300)
    plt.clf()
    plt.close()
...
if __name__ == "__main__":
    ...
    df['TWT'] = 1
    df = df.loc['2019-11-08':'2019-11-15']
